...
func FromURL(ctx context.Context, link string) (*Result, error) {
	u, err := url.Parse(link)
	if err != nil {
		return nil, err
	}
	link = u.String()
	body, err := tool.Get(ctx, link)
	if err != nil {
	...
	if len(m3u8.MasterPlaylist) != 0 {
		...
		return FromURL(ctx, tool.ResolveURL(u, sf.URI))
	}
